...
<template>
  <ListboxButton
      v-slot="{ open }"
      class="relative flex w-full items-center justify-between truncate border border-gray-300 px-3 py-2 text-left leading-tight rounded-md"
  >
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="flex flex-col">
        <span class="text-sm text-gray-950" :title="selectedElementLabel">
          {{ selectedElementLabel }}
        </span>
        <span class="text-xs text-dialog" :title="label">
          {{ label }}
        </span>
      </div>
    </div>
